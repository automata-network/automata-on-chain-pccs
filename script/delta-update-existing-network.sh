...
NEW_DAO_V2=$(jq -r ".AutomataFmspcTcbDaoVersionedV2_tcbeval_${TCB_EVAL}" "$PCCS_REPO/deployment/$CHAIN_ID.json")

echo "[4.5/7] Grant StorageV2 access to new FmspcTcbDaoVersionedV2"
OWNER="$OWNER_ADDR" forge script script/automata/ConfigAutomataDao.s.sol:ConfigAutomataDao \
  --rpc-url "$RPC_URL" \
  "${FORGE_AUTH_ARGS[@]}" \
  --broadcast --skip-simulation -vv \
  --sig "grantDaoV2(address)" "$NEW_DAO_V2"

echo "[5/7] Grant attester role to new V2 DAO"
