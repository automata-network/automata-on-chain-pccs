...
import "../../src/example/AccessControlledPCCS.sol";

contract ConfigACScript is Script {
    using JSONParserLib for JSONParserLib.Item;
    using LibString for string;

    address allDaoAddress = vm.envAddress("AC_PCCS");
    AccessControlledPCCS public allDao = AccessControlledPCCS(allDaoAddress);

    // NOTE: Subject to change
    string internal constant tcbInfoPath = "/script/example/assets/0524/tcb.json";
    string internal constant idPath = "/script/example/assets/0524/identity.json";
    ...
    function run() public {
        allDao = AccessControlledPCCS(allDaoAddress);
        ...
        allDao.upsertPcsCertificates(CA.SIGNING, signingDer);

        // upsert Platform intermediate CA
        allDao.upsertPcsCertificates(CA.PLATFORM, platformDer);

        // upsert fmspc tcb
        TcbInfoJsonObj memory tcbInfoJson = _readTcbInfoJson(tcbInfoPath);
        allDao.upsertFmspcTcb(tcbInfoJson);

        // upsert enclave identity
        EnclaveIdentityJsonObj memory identityJson = _readIdentityJson(idPath);
    ...
    function configureQeIdentityFromPath(uint256 teeType, uint256 version, string memory path) public {
        EnclaveIdentityJsonObj memory identityJson = _readIdentityJson(path);

        vm.broadcast(key);
        allDao.upsertEnclaveIdentity(teeType, version, identityJson);
    }

    function _readTcbInfoJson(string memory path) private view returns (TcbInfoJsonObj memory tcbInfoJson) {
        string memory inputFile = string.concat(vm.projectRoot(), path);
    ...
    function _readIdentityJson(string memory path) private view returns (EnclaveIdentityJsonObj memory identityJson) {
        string memory inputFile = string.concat(vm.projectRoot(), path);
