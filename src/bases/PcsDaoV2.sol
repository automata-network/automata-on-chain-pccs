...
import {X509CRLHelperV2, X509CRLMetadata} from "../helpers/X509CRLHelperV2.sol";
...
abstract contract PcsDaoV2 is DaoBase, SigVerifyBase {
    ...
    error Invalid_Authority_Key_Identifier();
    error Missing_Crl(CA ca);
    error Invalid_Stored_Crl(CA ca);
    ...
    function _upsertPcsCrl(CA ca, bytes calldata crl) private returns (bytes32 attestationId) {
        (bytes32 hash, bytes32 key, X509CRLMetadata memory currentCrl) = _validatePcsCrl(ca, crl);
        ...
        returns (bytes32 hash, bytes32 key, X509CRLMetadata memory currentCrl)
    {
        ...
        _validatePcsCrlMetadata(ca, currentCrl, true);
    }

    function _validatePcsCrlMetadata(CA ca, X509CRLMetadata memory currentCrl, bool checkRollback) private view {
        ...
        if (checkRollback) {
            (uint64 existingCrlNotValidBefore,) = _loadPcsValidity(PCS_KEY(ca, true));
        ...
        // Step 4: Bind the CRL to the configured issuer certificate. The
        // signature alone authenticates the TBS, but the AKI/SKI comparison
        // prevents accepting a CRL under the wrong configured CA identity.
        bytes memory issuerCert = _getIssuer(ca);
        bytes memory issuerSubjectKeyIdentifier = X509Helper(x509).getSubjectKeyIdentifier(issuerCert);
        if (
            currentCrl.authorityKeyIdentifier.length == 0 || issuerSubjectKeyIdentifier.length == 0
                || !BytesUtils.compareBytes(currentCrl.authorityKeyIdentifier, issuerSubjectKeyIdentifier)
        ) {
            revert Invalid_Authority_Key_Identifier();
        }

        // Step 5: Verify the signature over the exact TBS bytes hashed by the helper.
        bool sigVerified = verifySignature(currentCrl.tbsSha256, currentCrl.signature, issuerCert);
        if (!sigVerified) {
            revert Invalid_Signature();
        }
    }
