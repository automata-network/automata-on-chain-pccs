...
/**
 * @title FmspcTcb Helper V3 — reverse serializer + ranged parser
 * @notice Used by FmspcTcbDaoV3 to verify that offchain-uploaded packed level bytes are
...
contract FmspcTcbHelperV3 {
    ...
    /// @notice Single-key variant of `findArrayBounds`. Used by FmspcTcbDaoV3's staged
    ...
    /// @notice Single-key variant for tdxModuleIdentities. Only called by V3 DAO on TDX paths
    ...
    /// (`FmspcTcbHelperV3ParityTest`) asserts this against every published SGX+TDX fmspc.
    ...
    /// chained with the previous batch. The caller (V3 DAO) tracks `expectedFirstByteStart`
    ...
    /// @param stream concatenated batches in the FmspcTcbDaoV3 inline identity format:
